...
class WaitkTransformerModel(FairseqEncoderDecoderModel):
    ...
    def get_word_orcale_tokens(self, pred_logits, prev_output_tokens, update_num = None,epsilon=1e-6):
        ...
        sample_gold_prob = self.decay_prob(i = update_num, k=self.decay_k)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, update_num=None,**kwargs):
        ...
        if update_num is not None:
            with torch.no_grad():
                ...
                prev_output_tokens = self.get_word_orcale_tokens(decoder_out[0].detach(), prev_output_tokens,update_num = update_num )
        decoder_out = self.decoder.forward_train(prev_output_tokens, encoder_out=encoder_out,**kwargs)
        return decoder_out
    ...
    def build_model(cls, args, task):
        ...
        return WaitkTransformerModel(args, encoder, decoder)
